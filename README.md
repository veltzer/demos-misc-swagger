<!-- This file is generated. Do not edit it by hand: your changes will be
     lost on the next build. Edit the template it is rendered from, or the
     values that template reads, and build again. -->
# *demos-misc-swagger* project by Mark Veltzer

description: Demos on how to use swagger

project website: https://veltzer.github.io/demos-misc-swagger

author: Mark Veltzer

version: 0.0.1

![Code style: black](https://img.shields.io/badge/code%20style-black-000000.svg)

## github

![License](https://img.shields.io/github/license/veltzer/demos-misc-swagger)

## build

![build](https://github.com/veltzer/demos-misc-swagger/workflows/build/badge.svg)

## contact me

[mark.veltzer@gmail.com](mailto:mark.veltzer@gmail.com)

Mark Veltzer, Copyright © 2023, 2024, 2025, 2026
